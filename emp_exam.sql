...
select ename employee, sal as "Monthly Salay" from emp where deptno in (10,30) and sal >1500;
...
select empno, ename, sal, sal*1.15 as "New Salary", sal*1.15 - sal Increase from emp;
 
--18> 모든 사원의 이름(첫 글자는 대문자로, 나머지 글자는 소문자로 표시) 및 이름 길이를
-- 표시하는 쿼리를 작성하고 컬럼 별칭은 적당히 넣어서 출력하라.
select initcap(ename), length(ename) as "Name Length" from emp;
...
--(단, 직업 목록이 중복되지 않게 하라.)
select distinct e.job, d.loc from emp e join dept d on e.deptno=d.deptno and d.deptno='30';
 
--23> 커미션이 책정되어 있는 모든 사원의 이름, 부서이름 및 위치를 출력하라.
select e.ename, d.dname, d.loc from emp e join dept d on e.deptno = d.deptno and comm is not null;
...
select e.ename, d.dname from emp e join dept d on e.ename like'A%';
